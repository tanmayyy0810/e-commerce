...
const PlaceOrder = () => {
  ...
  const totalAmount = getCartAmount() + delivery_charge();
  ...
  const onSubmitHandler = async (event)=> {
    ...
    try {
      ...
      // console.log("Delivery Fee:", delivery_charge);
      switch(method){
        // api calls for cod
        case 'cod':
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex flex-col md:flex-row gap-3'>
            <div onClick={()=>setMethod('stripe')} className='flex items-center gap-3 border p-3 cursor-pointer w-full md:w-auto'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method==='stripe'?'bg-green-400': ''}`}></p>
              <img className='h-5' src={assets.stripe_logo} alt=''/>
            </div>
            <div onClick={()=>setMethod('razorpay')} className='flex items-center gap-3 border p-3 cursor-pointer w-full md:w-auto'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method==='razorpay'?'bg-green-400': ''}`}></p>
              <img className='h-5' src={assets.razorpay_logo} alt=''/>
            </div>
            <div onClick={()=>setMethod('cod')} className='flex items-center gap-3 border p-3 cursor-pointer w-full md:w-auto'>
